...
// hash round constants，每次hash需要運算80個rounds
uint8_t *PadInput(const uint8_t *inputBytes, size_t inputLength, size_t &paddedLength)
{
    size_t originalLength = inputLength;
    size_t paddingLength = (BlockSize - ((originalLength + 16) % BlockSize)); // 預留至少16Bytes做填充用，讓我寫入1000 0000作為開始填充的標誌
...
void shaCompress(uint64_t hash[8], const uint8_t block[BlockSize])
{
}
